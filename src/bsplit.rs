...
pub fn binary_split(
    ...
    lookup_depth: usize,
    lookup: &HashMap<[usize; 2], (NumFac, NumFac, NumFac)>,
    ...
    if b - a == 1 {
        ...
        // lookup_depth, lookup); let (p2, q2, r2) = binary_split(mid,
        // b, depth + 1, sieve, lookup_depth, lookup);

        let (p1, q1, r1) = if depth == lookup_depth {
            // lookup.remove(&[a, mid]).unwrap()
            lookup.get(&[a, mid]).unwrap()
        } else {
...
//     sieve: &PrimeFactorSieve,
//     lookup_depth: usize,
//     mut lookup: Option<HashMap<[usize; 2], (NumFac, NumFac, NumFac)>>,
// ) -> (NumFac, NumFac, NumFac) {
//     let mut stack = VecDeque::new();
//     stack.push_back((a, b, depth));
//
//     let mut ranges: Vec<[usize; 2]> = Vec::with_capacity((b - a).ilog2() as
// usize + 1);
//
...
//                 .map(|[a, b]| {
//                     let (p1, q1, r1) =
//                         binary_split(a, b, 0, sieve, usize::MAX, &mut
// HashMap::new());                     ([a, b], (p1, q1, r1))
//                 })
//                 .collect(),
//         );
//     }
//
//     binary_split(a, b, depth + 1, sieve, lookup_depth, &mut lookup.unwrap())
// }
...
pub fn par_split(
    ...
    sieve: &PrimeFactorSieve,
    max_depth: usize,
) -> HashMap<[usize; 2], (NumFac, NumFac, NumFac)> {
    let mut stack = VecDeque::new();
    stack.push_back((a, b, 0));

    let mut ranges: Vec<[usize; 2]> =
        Vec::with_capacity((b - a).ilog2() as usize + 1);

    while let Some((a_, b_, depth_)) = stack.pop_back() {
        if depth_ == depth {
            ranges.push([a_, b_]);
            continue;
        }
        ...
        let mid = (a_ + b_) / 2;
        stack.push_back((a_, mid, depth_ + 1));
        stack.push_back((mid, b_, depth_ + 1));
    }
    ...
    if depth == max_depth {
        ...
        let lookup = par_split(a, b, depth + 1, sieve, max_depth);
        ...
        ranges
            ...
            .map(|[a, b]| {
                let (p1, q1, r1) =
                    binary_split(a, b, depth, sieve, depth, &lookup);
                ([a, b], (p1, q1, r1))
            })
            .collect::<HashMap<_, _>>()
    }
